http://github.com/flatironinstitute/catvae/blob/master/catvae/sim.py
import numpy as np
...
def multinomial_batch_bioms(k, D, N, M, C=2,
    ...
    sigma = np.sqrt(sigma_sq)
    #B is batch effect
    z = np.random.normal(size=(total, hdims))+B[batch_idx[i]
    eta = np.random.normal(np.matmul(z, W.T), sigma).astype(np.float32)
    # Create ILR basis
    tree = random_linkage(D)
    Psi = _balance_basis(tree)[0]
    # add batch effects
    # alpha with size k is modeling the batch effects in lower dimension
    alpha = np.abs(np.random.normal(0, 0.5, size=(k)))
    ...
    eta = np.vstack([eta[i]] for i in range(N)])
